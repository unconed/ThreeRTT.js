...
j.call(a,Math.max(a.length-c,0)):a[a.length-1]};c.rest=c.tail=function(a,c,b){return j.call(a,null==c||b?1:c)};c.compact=function(a){return c.filter(a,function(a){return!!a})};c.flatten=function(a,f){return c.reduce(a,function(a,b){if(c.isArray(b))return a.concat(f?b:c.flatten(b));a[a.length]=b;return a},[])};c.without=function(a){return c.difference(a,j.call(arguments,1))};c.uniq=c.unique=function(a,b,d){var d=d?c.map(a,d):a,e=[];3>a.length&&(b=!0);c.reduce(d,function(d,k,g){if(b?c.last(d)!==k||
...
ThreeRTT.FragmentMaterial=function(d,b,e,g){if(e instanceof Array){var h={};_.each(e,function(b){h["texture"+(i+1)]=b});e=h}else e&&(e={texture1:e});d instanceof Array||(d=[d]);d=_.map(d,function(b){return ThreeRTT.toTarget(b)});g=_.extend(g||{},{sampleStep:{type:"v2",value:new THREE.Vector2}});_.each(e,function(b,d){g[d]={type:"t",value:ThreeRTT.toTexture(b)}});_.each(d,function(b,d){var e="texture"+(d+1);g[e]||(g[e]={type:"t",value:b.read()})});g.texture1&&!g.texture&&(g.texture=g.texture1);d[0].on("render",
function(){console.log("on render",d[0].width,d[0].height);var b=g.sampleStep.value;b.x=1/(d[0].width-1);b.y=1/(d[0].height-1)});b=new THREE.ShaderMaterial({uniforms:g,vertexShader:ThreeRTT.getShader("generic-vertex-screen"),fragmentShader:ThreeRTT.getShader(b||"generic-fragment-texture")});b.side=THREE.DoubleSide;return b};
